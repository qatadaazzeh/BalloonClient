...
import { useDeliveryProcessing } from "@/hooks/useDeliveryProcessing";
import { useStableColorMapping } from "@/hooks/useStableColorMapping";
...
const Index = memo(() => {
  const { printBalloonDelivery } = useContestConnection();
  const { contestData, isLoading, isConnected } = useContestData();
  ...
  const { getTeamById, getProblemById } = useStableColorMapping({
    teams: contestData?.teams || [],
    problems: contestData?.problems || [],
  });

  const { deliveries: processedDeliveries } = useDeliveryProcessing({
    contestData,
    deliveredBalloons,
  });

  // Update allDeliveries when processedDeliveries changes
  useEffect(() => {
    const prevCount = allDeliveries.length;
    console.log(
      `📊 useEffect triggered: processedDeliveries.length=${processedDeliveries.length}, prevCount=${prevCount}`
    );
    setAllDeliveries(processedDeliveries);
    const newCount = processedDeliveries.length;

    if (newCount > prevCount) {
      console.log(
        `🎈 Added ${newCount - prevCount} new balloon(s) to UI (Total: ${newCount})`
      );
    } else if (newCount !== prevCount) {
      console.log(
        `🔄 Deliveries count changed from ${prevCount} to ${newCount}`
      );
    } else {
      console.log(
        `ℹ️ No count change, but processedDeliveries array may have updated`
      );
    }
  }, [processedDeliveries]);
  ...
  const markAsDelivered = useCallback(
    async (deliveryId: string) => {
      ...
      setDeliveredBalloons((prev) => new Set([...prev, deliveryKey]));

      setAllDeliveries((prev) =>
        prev.map((d) =>
          d.id === deliveryId
            ? { ...d, status: "delivered" as const, deliveredAt }
            : d
        )
      );
    ...
    [allDeliveries, contestData, printBalloonDelivery]
  );
                ...
                {pendingDeliveries.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {pendingDeliveries.map((delivery) => {
                      const team = getTeamById(delivery.teamId);
                      const problem = getProblemById(delivery.problemId);
